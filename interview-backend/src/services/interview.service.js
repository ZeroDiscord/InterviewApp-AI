...
const finalizeAndGenerateReport = async (sessionId) => {
    ...
    // Convert scores from percentage (0-100) back to decimal (0-1) for averaging
    const overallScore = responses.reduce((acc, r) => acc + (r.aiScore / 100), 0) / responses.length;
    const aiSummary = await generateFinalReport(responses);
    
    const report = await InterviewReport.create({
        session: sessionId,
        overallScore: (overallScore * 100).toFixed(2), // Convert back to percentage for storage
        strengths: aiSummary.strengths, // This is now an array
        areasForImprovement: aiSummary.areasForImprovement, // This is now an array
