...
const transporter = nodemailer.createTransport(
  smtpTransport({
    ...
    auth: {
      user: 'anchaljak2418@gmail.com',
      pass: 'lthq vdmp jgmj ewhg'
    }
  })
);
...
const sendMail = async (mailOptions) => {
  try {
    const baseOptions = {
      from: '"AI Interview Portal" <anchaljak2418@gmail.com>'
    };
    ...
    html: `
      <div style="${mainContainerStyle}">
        <div style="${contentWrapperStyle}">
          <div style="${headerStyle}"><h1 style="${headerTextStyle}">AI Interview Portal</h1></div>
          <div style="${bodyStyle}">
          ...
          <div style="${footerStyle}">  ${new Date().getFullYear()} | AI Interview Portal</div>
        </div>
      </div>
    `
  };
    ...
    html: `
      <div style="${mainContainerStyle}">
        <div style="${contentWrapperStyle}">
          ...
          <div style="${footerStyle}">© ${new Date().getFullYear()} AI Interview Portal. All Rights Reserved.</div>
        </div>
      </div>
    `
  };
...
const sendDecisionEmail = async (to, decision, comments, candidateName) => {
  const subject = `An Update on Your Interview with AI Interview Portal`;
